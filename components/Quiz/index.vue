<template>
  <div class="lg:mt-12 px-8 lg:px-32 flex flex-col items-center">
    ...
    <p class="mt-4 mb-8 font-mono text-lg lg:text-xl lg:mb-14 lg:mt-8 xl:text-2xl">
    ...
    <div v-if="!showResult" v-for="question in QUESTIONS" class="w-full select-none">
      <div v-if="currentQuestion === question.id" class="flex flex-col items-center w-full">
        <p class="mb-4 text-xl xl:text-3xl font-mono">{{ question.title }}</p>
        <!-- ANSWER BUTTONS  -->
        <ul class="flex flex-col items-center space-y-2 xl:space-y-5 w-full xl:text-xl">
          <li class="p-4 xl:p-8 rounded-lg shadow-lg w-full flex justify-center" :class="{'bg-green-700 text-white': showAnswer && question.rightAnswer === 'A', 'bg-red-700 text-white': showAnswer && question.rightAnswer !== 'A', 'bg-white': !showAnswer}" @click="checkInput('A', question.rightAnswer)">
            {{ question.questionA }}
          </li>
          <li class="p-4 xl:p-8 rounded-lg shadow-lg w-full flex justify-center" :class="{'bg-green-700 text-white': showAnswer && question.rightAnswer === 'B', 'bg-red-700 text-white': showAnswer && question.rightAnswer !== 'B', 'bg-white': !showAnswer}" @click="checkInput('B', question.rightAnswer)">
            {{ question.questionB }}
          </li>
          <li class="p-4 xl:p-8 rounded-lg shadow-lg w-full flex justify-center" :class="{'bg-green-700 text-white': showAnswer && question.rightAnswer === 'C', 'bg-red-700 text-white': showAnswer && question.rightAnswer !== 'C', 'bg-white': !showAnswer}" @click="checkInput('C', question.rightAnswer)">
            {{ question.questionC }}
          </li>
          <li class="p-4 xl:p-8 rounded-lg shadow-lg w-full flex justify-center" :class="{'bg-green-700 text-white': showAnswer && question.rightAnswer === 'D', 'bg-red-700 text-white': showAnswer && question.rightAnswer !== 'D', 'bg-white': !showAnswer}" @click="checkInput('D', question.rightAnswer)">
            {{ question.questionD }}
          </li>
        </ul>
